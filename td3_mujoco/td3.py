...
def main():
    ...
    optimizer_actor = optim.Adam(
        actor_params, lr=args.lr, weight_decay=args.weight_decay
    )
    optimizer_critic = optim.Adam(
        critic_params, lr=args.lr, weight_decay=args.weight_decay
    )
    ...
    with wandb.init(project="TD3_TorchRL", name=args.exp_name, config=args):
        for i, tensordict in enumerate(collector):
            ...
            if collected_frames >= args.init_random_frames:
                ...
                for i in range(args.env_per_collector*args.optim_steps_per_batch):
                    ...
                    if i % args.policy_update_delay == 0:
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number delayed of optimization steps when the actor gets updated. Default: 2",
    )
    parser.add_argument(
